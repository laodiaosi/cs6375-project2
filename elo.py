...
if __name__ == '__main__':
    ...
    with open('/content/16-17Result.csv', 'w') as f:
        writer = csv.writer(f)
        writer.writerow(['win', 'lose', 'probability'])
        writer.writerows(result)
        print('done.')
    pd.read_csv('/content/16-17Result.csv', header=0)
    a = pd.read_csv(folder + '/16-17Result.csv')
    a.head()
    a = a["probability"]
    b = pd.read_csv(folder + '/16-17Result.csv')
    b = b["probability"]
    print(a.shape)
    print(b.shape)
